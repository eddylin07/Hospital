...
public class PatientController {
    ...
    public JSONObject appointment(@RequestBody Appointment appointment,HttpSession session){
        JSONObject json=new JSONObject();
        Login login=(Login)session.getAttribute("login");
        Patient patientInfo=patientService.findPatientByLoginId(login.getId());
        if(patientInfo==null){
            json.put("message","预约失败");
            return json;
        }
        appointment.setPatientid(patientInfo.getId());
        Patient patient=new Patient();
        String message=appointmentService.addAppointment(appointment);
        patient.setAppointmentid(appointmentService.selectTheLastAppointment(patientInfo.getId()));

...
public class DoctorController {
    ...
    public JSONObject zation(@RequestBody Hospitalization hospitalization,HttpSession session){
        JSONObject json=new JSONObject();
        if(hospitalization==null||!canCurrentDoctorAccessPatient(session,hospitalization.getPatientid())){
    ...
    public JSONObject printseek(@PathVariable Integer id,HttpSession session){
        ...
        if(doctor==null||!canDoctorAccessPatient(doctor,id)){
            json.put("message", CommonService.upd_message_error);
            return json;
        }
    ...
    private boolean canCurrentDoctorAccessPatient(HttpSession session,Integer patientId){
        Doctor doctor=getCurrentDoctor(session);
        return doctor!=null&&canDoctorAccessPatient(doctor,patientId);
    }

    private boolean canDoctorAccessPatient(Doctor doctor,Integer patientId){
    ...
    private Integer getPatientId(Map map){
        if(map==null||map.get("patientid")==null){
            return null;
        }
        Object patientId=map.get("patientid");
        if(patientId instanceof Number){
            return ((Number) patientId).intValue();
        }
        try{

...
public class DoctorController {
    ...
    public String seek(@PathVariable Integer id,HttpServletRequest request,HttpSession session){
        if(!canAccessPatient(session,id)){
            return "redirect:/hospital/doctor/index";
        }
    ...
    public JSONObject printseek(@PathVariable Integer id,HttpSession session){
        ...
        Doctor doctor=doctorService.getDoctorByLoginId(login.getId());
        JSONObject json=new JSONObject();
        if(doctor==null||!appointmentService.hasAppointmentWithDoctor(doctor.getId(),id)){
            json.put("message","无权操作该患者");
            return json;
        }
    ...
    private boolean canAccessPatient(HttpSession session,Integer patientId){
        ...
        Doctor doctor=doctorService.getDoctorByLoginId(login.getId());
        return doctor!=null&&appointmentService.hasAppointmentWithDoctor(doctor.getId(),patientId);
    }

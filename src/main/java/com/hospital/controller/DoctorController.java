...
public class DoctorController {
    ...
    public JSONObject drug(@RequestBody Map map,HttpSession session){
        JSONObject json=new JSONObject();
        Integer patientId=parseInteger((String)map.get("patientid"));
        if(patientId==null||!canTreatPatient(session,patientId)){
    ...
    public JSONObject zation(@RequestBody Hospitalization hospitalization,HttpSession session){
        JSONObject json=new JSONObject();
        if(hospitalization.getPatientid()==null||!canTreatPatient(session,hospitalization.getPatientid())){
            json.put("message","无权操作该患者");
            return json;
        }
    ...
    private boolean canTreatPatient(HttpSession session,Integer patientId){

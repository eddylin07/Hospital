...
public class DoctorController {
    ...
    public JSONObject zation(@RequestBody Hospitalization hospitalization,HttpSession session){
        JSONObject json=new JSONObject();
        if(hospitalization.getPatientid()==null || !canAccessPatient(session,hospitalization.getPatientid())){
            json.put("message","无权操作该患者");
            return json;
        }
    ...
    public JSONObject printseek(@PathVariable Integer id,HttpSession session){
        ...
        Doctor doctor=doctorService.getDoctorByLoginId(login.getId());
        JSONObject json=new JSONObject();
        if(doctor==null || !canAccessPatient(doctor.getId(),id)){
            json.put("message","无权操作该患者");
            return json;
        }
    ...
    private boolean canAccessPatient(HttpSession session, Integer patientId) {
        ...
        Doctor doctor=doctorService.getDoctorByLoginId(login.getId());
        return doctor!=null && canAccessPatient(doctor.getId(), patientId);
    }

    private boolean canAccessPatient(Integer doctorId, Integer patientId) {

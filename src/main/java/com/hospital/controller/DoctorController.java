...
public class DoctorController {
    ...
    public String seek(@PathVariable Integer id,HttpServletRequest request,HttpSession session){
        if(!authorizedDoctorForPatient(session,id)){
    ...
    public JSONObject drug(@RequestBody Map map,HttpSession session){
        ...
        if(patientid==null||!authorizedDoctorForPatient(session,patientid)){
    ...
    public JSONObject zation(@RequestBody Hospitalization hospitalization,HttpSession session){
        JSONObject json=new JSONObject();
        if(hospitalization.getPatientid()==null||!authorizedDoctorForPatient(session,hospitalization.getPatientid())){
            json.put("message","无权操作该患者");
            return json;
        }
    ...
    public JSONObject printseek(@PathVariable Integer id,HttpSession session){
        ...
        Doctor doctor=doctorService.getDoctorByLoginId(login.getId());
        JSONObject json=new JSONObject();
        if(doctor==null||!authorizedDoctorForPatient(session,id)){
            json.put("message","无权操作该患者");
            return json;
        }
        Seek seek=seekService.getSeekByPatientId(id);
        if(seek==null){
            json.put("message","暂无就诊单");
            return json;
        }
    ...
    private boolean authorizedDoctorForPatient(HttpSession session,Integer patientid){
        Login login=(Login)session.getAttribute("login");
        if(login==null||patientid==null){
            return false;
        }
        Doctor doctor=doctorService.getDoctorByLoginId(login.getId());
        if(doctor==null){

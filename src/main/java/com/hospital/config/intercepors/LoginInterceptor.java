...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    private boolean isAuthorized(String uri,Login login){
        if(uri.startsWith("/admin/")||uri.equals("/hospital/admin/index")){
            return hasRole(login,ROLE_ADMIN);
        }
        if(uri.equals("/hospital/doctor/index")||isProtectedDoctorPath(uri)){
            return hasRole(login,ROLE_DOCTOR);
        }
        if(uri.startsWith("/patient/")||uri.equals("/hospital/patient/index")){
            return hasRole(login,ROLE_PATIENT);
        }
        return true;
    }

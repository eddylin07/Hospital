...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(path.startsWith("/admin/")&&!hasRole(login,ROLE_ADMIN)){
            response.sendError(HttpServletResponse.SC_FORBIDDEN);
            return false;
        }
        if(isProtectedDoctorPath(path)&&!hasRole(login,ROLE_DOCTOR)){
            response.sendError(HttpServletResponse.SC_FORBIDDEN);
            return false;
        }
        if(path.startsWith("/patient/")&&!hasRole(login,ROLE_PATIENT)){
            response.sendError(HttpServletResponse.SC_FORBIDDEN);
            return false;
        }
    ...
    private boolean isProtectedDoctorPath(String path){

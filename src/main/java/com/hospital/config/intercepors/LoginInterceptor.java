...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    private boolean hasRequiredRole(HttpServletRequest request, Login login) {
        ...
        if (path.startsWith("/patient/")) {
            return Integer.valueOf(ROLE_PATIENT).equals(role);
        }
        if (isDoctorWorkflowPath(request, path)) {
            return Integer.valueOf(ROLE_DOCTOR).equals(role);
        }
        return true;
    }

    private boolean isDoctorWorkflowPath(HttpServletRequest request, String path) {
        if (!path.startsWith("/doctor/")) {
            return false;
        }
        String suffix = path.substring("/doctor/".length());
        if ("GET".equalsIgnoreCase(request.getMethod()) && !suffix.contains("/")
                && !"seekMedicalAdvice".equals(suffix)) {
            return false;
        }
        return true;
    }

...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    private boolean hasRequiredRole(HttpServletRequest request,Login login){
        ...
        if(path.startsWith("/patient/")&&!path.startsWith("/patient/search")){
            return Integer.valueOf(3).equals(role);
        }
        if("/hospital/patient/index".equals(path)){
            return Integer.valueOf(3).equals(role);
        }
        if("/hospital/doctor/index".equals(path)){
            return Integer.valueOf(2).equals(role);
        }
        return !isDoctorWorkflow(path)||Integer.valueOf(2).equals(role);
    }

    private boolean isDoctorWorkflow(String path){
        return path.startsWith("/doctor/seekMedicalAdvice")
                ||path.startsWith("/doctor/seek/")
                ||path.startsWith("/doctor/drug")
                ||path.startsWith("/doctor/zation")
                ||path.startsWith("/doctor/medicalhistory/")
                ||path.startsWith("/doctor/seekinfo")

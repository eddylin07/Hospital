...
public class PatientServiceImpl implements PatientService {
    ...
    public String seek(Patient patient) {
        ...
        Map<Drugs, Integer> updates = new HashMap<>();
        for(String drug:drugsids.split(",")){
            ...
            updates.put(drugs, drugnumber);
            price=price.add(drugs.getPrice().multiply(BigDecimal.valueOf(drugnumber)));
        }
        for (Map.Entry<Drugs, Integer> update : updates.entrySet()) {
            Drugs drugs = update.getKey();
            drugs.setNumber(update.getValue());

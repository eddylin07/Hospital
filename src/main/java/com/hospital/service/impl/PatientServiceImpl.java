...
public class PatientServiceImpl implements PatientService {
    ...
    public String seek(Patient patient) {
        ...
        for(String drug:drugsids.split(",")){
          ...
          orders.add(new DrugOrder(drugs,drugnumber));
        ...
        for(DrugOrder order:orders){
            order.drugs.setNumber(order.number);
            if(drugsMapper.updateNumber(order.drugs)<=0){
                rollbackCurrentTransaction();
                return "对不起"+order.drugs.getName()+"数量不足";
            }
        }
        seek.setPrice(price);
    ...
    private static class DrugOrder {
        ...
        private DrugOrder(Drugs drugs, Integer number) {
            this.drugs = drugs;
            this.number = number;
        }
    }

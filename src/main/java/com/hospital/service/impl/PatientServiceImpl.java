...
public class PatientServiceImpl implements PatientService {
    ...
    public String seek(Patient patient) {
        ...
        for(String drug:drugsids.split(",")){
          ...
          if (drugs == null || drugnumber <= 0 || drugs.getNumber() == null || drugs.getNumber() < drugnumber) {
              return "对不起药品数量不足";
          }
          BigDecimal drugprice=drugs.getPrice();
          Drugs requestedDrug = new Drugs();
          requestedDrug.setId(drugId);
          requestedDrug.setNumber(drugnumber);
          requestedDrugs.add(requestedDrug);
          price=price.add(drugprice.multiply(BigDecimal.valueOf(drugnumber)));
        }
        for (Drugs requestedDrug : requestedDrugs) {
            if (drugsMapper.updateNumber(requestedDrug) <= 0) {

...
public class PatientServiceImpl implements PatientService {
    ...
    public String seek(Patient patient) {
        ...
        Map<Drugs, Integer> drugsNumbers = new LinkedHashMap<>();
        for(String drug:drugsids.split(",")){
          ...
          drugsNumbers.put(drugs, drugnumber);
        }
        boolean drugsUpdated = true;
        for (Map.Entry<Drugs, Integer> entry : drugsNumbers.entrySet()) {
            Drugs drugs = entry.getKey();
            drugs.setNumber(entry.getValue());
            drugsUpdated = drugsMapper.updateNumber(drugs) > 0 && drugsUpdated;
        }
        seek.setPrice(price);
        return (drugsUpdated && patientMapper.updateByPrimaryKeySelective(patient) > 0 && seekMapper.updateDrugs(seek) > 0) ? CommonService.upd_message_success : CommonService.upd_message_error;
    }

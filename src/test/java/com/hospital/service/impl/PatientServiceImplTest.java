...
public class PatientServiceImplTest {
    ...
    @Test
    public void rejectsLaterOverdrawWithoutPartiallyDeductingEarlierDrug() {
        PatientServiceImpl service = new PatientServiceImpl();
        FakePatientMapper patientMapper = new FakePatientMapper();
        FakeDrugsMapper drugsMapper = new FakeDrugsMapper(10, 2);
        FakeSeekMapper seekMapper = new FakeSeekMapper();
        service.patientMapper = patientMapper;
        service.drugsMapper = drugsMapper;
        service.seekMapper = seekMapper;
        ...
        Patient patient = new Patient();
        patient.setId(8);
        patient.setDrugsids("1@1,2@3");

        assertEquals("对不起药品2数量不足", service.seek(patient));
        assertEquals(0, drugsMapper.updateCount);
        assertEquals(0, patientMapper.updateCount);
        assertEquals(0, seekMapper.updateCount);
    }
    ...
    private static class FakeDrugsMapper implements DrugsMapper {
        ...
        private final Integer secondDrugStock;
        private int updateCount;
        ...
        private FakeDrugsMapper(int stock, Integer secondDrugStock) {
            this.stock = stock;
            this.secondDrugStock = secondDrugStock;
        }
        ...
        public Drugs selectByPrimaryKey(Integer id) {
            ...
            drugs.setId(id);
            drugs.setName(id == 2 ? "药品2" : "阿司匹林");
            drugs.setPrice(new BigDecimal("10.0"));
            drugs.setNumber(id == 2 && secondDrugStock != null ? secondDrugStock : stock);
            return drugs;
        }

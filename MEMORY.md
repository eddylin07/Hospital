...
- On branch `cursor/missing-test-coverage-8bff`, tests cover failed/successful login session handling, dispensing price/inventory updates, insufficient-stock no-write behavior, latest-seek-only/null-safe prescription SQL, and `DrugsMapper` price/number/text mapping. `JAVA_HOME=/usr/lib/jvm/java-8-openjdk-amd64 mvn test` passed with 6 tests.
